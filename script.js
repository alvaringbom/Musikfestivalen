...
const fetchData = async () => {
  // asynkron funktion (async), hämtar data från API
  try {
    // API-anrop, skickar förfrågan
    const response = await fetch(url); // await, väntar på svar innan koden fortsätter

    if (!response.ok) {
      // kontroll om API-svaret INTE(!) är ok
    ...
    const artistFullInfo = data.items.map((artist) => {
      // array med objekt som innehåller all information
      ...
      return {
        ...
        dayDate: new Date(day.fields.date).toLocaleDateString("en-GB", {
          // för att få dagen på engelska, och utskriven på sättet jag vill
          day: "numeric",
          month: "long",
        }),
    ...
    const artistHTML = artistFullInfo
      .map((artist) => {
        // HTML-element för varje artist inkl alla delar
    ...
    //Stage HTML---------------------------
    const stages = [
      ...new Set(artistFullInfo.map((artist) => artist.stage)),
    ].sort(); // skapar lista över scener och sorterar alfabetiskt (sort)
    const stageContainer = document.getElementById("stage-container");
    const stageHTML = stages
      .map((stageName) => {
        const stageInfo = artistFullInfo.find(
          (artist) => artist.stage === stageName
        );
    ...
    // Program HTML ---------------------------
    const dayProgram = artistFullInfo.reduce((acc, artist) => {
      // gruppera artister per dag och lagras i objektet dayProgram
    ...
    const sortedDays = Object.keys(dayProgram).sort(
      (a, b) => dayOrder.indexOf(a) - dayOrder.indexOf(b)
    );

    const programContainer = document.getElementById("program-container");
    const programHTML = sortedDays
      .map((day) => {
    ...
    // om något går fel under try så fångas felet här
